...
void saveToFile() {
    ...
    for (int i = 0; i < cnt; i++) {
        outFile << Events[i].EventNumber << "|"
                << Events[i].gs << "|"
                << Events[i].time << "|"
                << Events[i].ven << "|"
                << Events[i].info << "|"
                << Events[i].usr.booking_id << "|"
                << Events[i].usr.name << "|"
                << Events[i].usr.address << "|"
                << Events[i].usr.phone << "|"
                << Events[i].usr.date << "|"
                << Events[i].usr.payment_advance << endl;
    }
    outFile.close();
}
...
void loadFromFile() {
    ...
    cnt = 0;
    string line;
    while (getline(inFile, line) && cnt < max) {
        stringstream ss(line);
        string token;
        Event tempEvent;

        getline(ss, token, '|');
        tempEvent.EventNumber = stoi(token);

        getline(ss, token, '|');
        tempEvent.gs = stoi(token);

        getline(ss, token, '|');
        tempEvent.time = stoi(token);

        getline(ss, token, '|');
        tempEvent.ven = token;

        getline(ss, token, '|');
        tempEvent.info = stoi(token);

        getline(ss, token, '|');
        tempEvent.usr.booking_id = stoi(token);

        getline(ss, token, '|');
        strcpy(tempEvent.usr.name, token.c_str());

        getline(ss, token, '|');
        strcpy(tempEvent.usr.address, token.c_str());

        getline(ss, token, '|');
        strcpy(tempEvent.usr.phone, token.c_str());

        getline(ss, token, '|');
        strcpy(tempEvent.usr.date, token.c_str());

        getline(ss, token, '|');
        tempEvent.usr.payment_advance = stof(token);

        Events[cnt++] = tempEvent;
    }
    inFile.close();
}
...
void EventMen::GuestSummary(char *uname) {
    ...
    if (z == 1) {
        ...
        cout << "\n Celebrate any occasion with us to make your event more special in reasonable price.\n You can also customize the decors.\n We work on any venue you prefer " << endl;
